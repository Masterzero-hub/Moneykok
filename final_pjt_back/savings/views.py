...
# 가입한 상품 해지하기
@api_view(["DELETE"])
def joined_savings_delete(request, joined_savings_id):
    try:
        user = request.user
         # 삭제할 상품 찾기
        try:
            joined_product = JoinedSavings.objects.get(pk=joined_savings_id, user=user)
        except JoinedSavings.DoesNotExist:
